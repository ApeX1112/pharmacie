...
import {
    Clock, Package, Truck, Users, Activity, AlertTriangle, Gauge, TrendingUp,
    Timer, BarChart3
} from 'lucide-react';

const KpiCard = ({ icon: Icon, label, value, unit, accent = 'indigo', small = false }) => {
    const accents = {
    ...
    return (
        <div className={`bg-white p-2.5 rounded-lg shadow-sm border-l-4 ${accents[accent] || accents.indigo}`}>
            <div className="flex items-center gap-1.5 mb-0.5">
                <Icon size={small ? 12 : 14} className="text-gray-400" />
                <p className="text-xs text-gray-500 truncate">{label}</p>
            </div>
            <div className="flex items-baseline gap-1">
                <p className={`${small ? 'text-lg' : 'text-xl'} font-bold text-gray-800`}>{value}</p>
...
const Dashboard = () => {
    ...
    const mm = Math.floor((hourRaw % 1) * 60);
        ...
        <div className="w-80 bg-gray-50 flex flex-col border-l border-gray-200 overflow-y-auto">
            ...
            <div className="bg-gradient-to-r from-slate-800 to-slate-700 text-white px-4 py-3">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-300 uppercase tracking-wider">Simulation</span>
                    </div>
                    <span className="text-xs text-gray-400">Jour {day}</span>
                </div>
                <p className="text-3xl font-mono font-bold tracking-wider mt-1">{timeStr}</p>
            </div>
            ...
            <div className="p-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                    <KpiCard icon={Package} label="Arrivées" value={metrics.totalArrivals} accent="indigo" small />
                    <KpiCard icon={Truck} label="Terminées" value={metrics.completedOrders} accent="green" small />
                    <KpiCard icon={Activity} label="Réappro." value={metrics.totalReplenishments || 0} accent="orange" small />
                    <KpiCard icon={Users} label="Agents" value={agents.length} accent="blue" small />
                </div>
                ...
                <div className="border-t border-gray-200 pt-2 mt-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <KpiCard
                            icon={Timer}
                            label="Moy. Prépa."
                            value={(metrics.avgPrepTime || 0).toFixed(1)}
                            unit="h"
                            accent="purple"
                            small
                        />
                        <KpiCard
                            icon={Clock}
                            label="Attente Ctrl"
                            value={(metrics.avgControlWaitTime || 0).toFixed(1)}
                            unit="h"
                            accent="amber"
                            small
                        />
                ...
                <div className="border-t border-gray-200 pt-2 mt-2">
                    <div className="bg-white p-3 rounded-lg shadow-sm border-l-4 border-emerald-500">
                        ...
                        <div className="flex items-baseline gap-1">
                            <p className="text-2xl font-bold text-gray-800">{(metrics.throughput || 0).toFixed(2)}</p>
                ...
                <div className="border-t border-gray-200 pt-2 mt-2">
                    <div className="bg-white p-2.5 rounded-lg shadow-sm">
                        ...
                        <p className="text-lg font-bold text-gray-700">{(metrics.totalDistance || 0).toFixed(0)} <span className="text-xs text-gray-400">m</span></p>
                    </div>
                </div>
            </div>
