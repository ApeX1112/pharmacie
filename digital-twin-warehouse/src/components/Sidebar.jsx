...
const Sidebar = () => {
    ...
    const handleAddRandomOrder = () => {
        ...
        const pickingZones = config.zones.filter(z => z.type === 'picking').map(z => z.id);
        const randomZone = pickingZones[Math.floor(Math.random() * pickingZones.length)];
        ...
        <>
            <div className="w-16 bg-slate-800 flex flex-col items-center py-4 space-y-4 text-white z-50 relative">
                ...
                <div className="flex flex-col items-center space-y-1">
                    ...
                    <button
                        onClick={() => setSpeed(5)}
                        className={`p-1 rounded ${simulationSpeed === 5 ? 'bg-blue-600' : 'hover:bg-slate-700'}`}
                    >
                        5x
                    </button>
                </div>
                ...
                <div className="flex flex-col items-center space-y-1">
                    ...
                    <input
                        ...
                        value={useWarehouseStore(state => state.parameters.inboundFrequency)}
                        onChange={(e) => useWarehouseStore.getState().setParameter('inboundFrequency', parseInt(e.target.value))}
                ...
                <div className="flex flex-col items-center space-y-1">
                    ...
                    <input
                        ...
                        value={useWarehouseStore(state => state.parameters.replenishThreshold)}
                        onChange={(e) => useWarehouseStore.getState().setParameter('replenishThreshold', parseInt(e.target.value))}
                        className="w-12 h-1 bg-gray-600 rounded-lg appearance-none cursor-pointer"
