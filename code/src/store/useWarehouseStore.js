...
const useWarehouseStore = create((set, get) => ({
  ...
  agents: [
    {
      ...
      y: 650,
      state: "idle",
      shifts: { start1: 6, end1: 12, start2: 13, end2: 17 },
    },
    {
      id: "M2",
      type: "Storekeeper",
      ...
      y: 400,
      state: "idle",
      shifts: { start1: 10, end1: 14, start2: 15, end2: 19 },
    },
    {
      id: "M3",
      type: "Storekeeper",
      ...
      y: 500,
      state: "idle",
      shifts: { start1: 14, end1: 18, start2: 19, end2: 22 },
    },
    {
      id: "P1",
      type: "Picker",
      ...
      y: 150,
      state: "idle",
      shifts: { start1: 6, end1: 20, start2: 0, end2: 0 },
    },
    {
      id: "P2",
      type: "Picker",
  ...
  updateAgentShifts: (id, newShifts) =>
    set((state) => ({
      agents: state.agents.map((a) =>
        a.id === id ? { ...a, shifts: newShifts } : a,
      ),
      initialAgents: state.initialAgents.map((a) =>
        a.id === id ? { ...a, shifts: newShifts } : a,
      ),
    })),
    ...
    set((state) => {
      ...
      const zones = state.layoutConfig.zones.map((z) => {
        if (z.type === "picking" && z.capacity) {
          return {
            ...z,
            stock: Math.floor(Math.random() * z.capacity * 0.4) + 5,
          };
        }
        if (z.type === "storage" && z.capacity) {
          return {
            ...z,
            stock: Math.floor(Math.random() * z.capacity * 0.6) + 50,
          };
        }
        return z;
      });
  ...
  updateZoneStock: (zonesOrId, amount) =>
    set((state) => {
      ...
      return {
        layoutConfig: {
          ...state.layoutConfig,
          zones: state.layoutConfig.zones,
        },
      };
    }),
  ...
  setArrivalParam: (zone, key, value) =>
    set((state) => ({
      parameters: {
        ...state.parameters,
        arrivalParams: {
    ...
    set((state) => ({
      ...
      metrics:
        status === "completed"
          ? {
              ...state.metrics,
              completedOrders: state.metrics.completedOrders + 1,
            }
          : state.metrics,
    })),
    ...
    set((state) => {
      ...
      return {
        parameters: JSON.parse(
          JSON.stringify(state.baselineSnapshot.parameters),
        ),
      ...
      switch (scenarioId) {
        ...
        case "minus1_picker": {
          ...
          const newAgents = state.agents.filter(
            (a) => a.id !== pickers[pickers.length - 1].id,
          );
        ...
        case "frequent_rupture": {
          ...
          return {
            layoutConfig: { ...state.layoutConfig, zones },
